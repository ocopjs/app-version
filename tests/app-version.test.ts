...
import { appVersionMiddleware, AppVersionProvider } from "../src/app-version";
...
describe("AppVersionProvider", () => {
  test("AppVersionProvider - simple access", async () => {
    ...
    expect(appVersion.getQueryResolvers({ schemaName })).toHaveProperty(
      "appVersion",
    );
    expect(appVersion.getQueryResolvers({ schemaName }).appVersion?.()).toEqual(
      "1.0.0",
    );
